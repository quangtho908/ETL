...
export class ExtractService {
  ...
  async getConfig() {
    ...
    this.configs = configsList.filter(configData => (configData.mapExtract !== null || configData.url !== null) && configData.enable)
  }
  ...
  async extract() {
    ...
    for(let config of this.configs) {
      ...
      for (let i = 0; i < allProductLinks.length; i += chunkSize) {
        ...
        const productsDetails = await Promise.all(chunk.map(link => this.getProductDetails(link, config)));
      ...
      writeFileSync(config.file, JSON.stringify(allProductsDetails, null, 2), 'utf-8');
    }
